...
public:
    Arms(): m_name(""), m_physical(0), m_magic(0), m_fire(0), m_lightning(0), m_total(0), m_weight(0.0)
    {
    }

    Arms(string name, int physical, int magic, int fire, int lightning, double weight): m_name(name), m_physical(physical),
    ...
    friend ostream& operator<< (ostream &out, const Arms &subjectArms);

    Arms& operator= (const Arms &sourceArms);
};
